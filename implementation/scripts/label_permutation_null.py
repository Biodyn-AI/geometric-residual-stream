...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Permutation null for geometry delta AUROC")
    ...
    parser.add_argument("--output-dir", type=Path, default=subproject_root / "implementation" / "outputs" / "cycle3_null")
...
def main() -> None:
    ...
    labels_true = edge_df["label"].to_numpy(dtype=np.int32)
    ...
    y_true = labels_true[valid]
    x_base = baseline[valid]
    x_plus = np.column_stack([x_base, geom[valid]])
    ...
    # Reference delta on true labels.
    probs_base_true = _crossval_probs(x_base, y_true, cv)
    probs_plus_true = _crossval_probs(x_plus, y_true, cv)
    true_delta = roc_auc_score(y_true, probs_plus_true) - roc_auc_score(y_true, probs_base_true)
    ...
    for perm_idx in range(args.n_perm):
        ...
        if np.unique(y_perm).size < 2:
            continue
        probs_base = _crossval_probs(x_base, y_perm, cv)
        probs_plus = _crossval_probs(x_plus, y_perm, cv)
        delta = roc_auc_score(y_perm, probs_plus) - roc_auc_score(y_perm, probs_base)
        perm_rows.append({"perm_idx": perm_idx, "delta_cv_auroc": float(delta)})

    perm_df = pd.DataFrame(perm_rows)
    perm_df.to_csv(output_dir / "label_permutation_delta_distribution.csv", index=False)

    p_value = float((perm_df["delta_cv_auroc"] >= true_delta).mean()) if not perm_df.empty else float("nan")
    ...
    summary.to_csv(output_dir / "label_permutation_summary.csv", index=False)

    print(f"[done] {output_dir / 'label_permutation_delta_distribution.csv'}")
    print(f"[done] {output_dir / 'label_permutation_summary.csv'}")
