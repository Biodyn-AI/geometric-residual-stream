...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "External-lung seed-ensemble bundle test: compare scGPT layer-bundle performance "
            "across seeds and with feature averaging across seeds."
        )
    )
    parser.add_argument(
        "--processed-h5ad",
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=subproject_root / "implementation" / "outputs" / "cycle20_external_lung_seed_ensemble_bundle_test",
    )
...
def main() -> None:
    ...
    # Compute per-seed per-layer scGPT features.
    ...
    # Seed-ensemble feature averaging per layer.
    for name, layers in bundles:
        valid = gf_valid.copy()
        for layer in layers:
            for seed in seeds:
        ...
        # Average features across seeds per layer, then stack layers.
        ens_layers = []
        for layer in layers:
            layer_stack = np.column_stack([seed_layer_feat[seed][layer][valid] for seed in seeds])
            ens_layers.append(np.mean(layer_stack, axis=1))
        sc_ens_stack = np.column_stack(ens_layers)
        gf = gf_feat[valid].reshape(-1, 1)

        auc_base = _crossval_auc(x_base, y, args.cv_splits, args.cv_repeats, args.seed)
        auc_sc = _crossval_auc(np.column_stack([x_base, sc_ens_stack]), y, args.cv_splits, args.cv_repeats, args.seed)
        auc_gf = _crossval_auc(np.column_stack([x_base, gf]), y, args.cv_splits, args.cv_repeats, args.seed)
        auc_both = _crossval_auc(
            np.column_stack([x_base, sc_ens_stack, gf]), y, args.cv_splits, args.cv_repeats, args.seed
        )
        ...
        rows.append(
            {
                ...
                "baseline_plus_both_cv_auroc": auc_both,
                "scgpt_delta_over_base": auc_sc - auc_base if np.isfinite(auc_sc) and np.isfinite(auc_base) else float("nan"),
                "geneformer_delta_over_base": auc_gf - auc_base if np.isfinite(auc_gf) and np.isfinite(auc_base) else float("nan"),
                "both_delta_over_base": auc_both - auc_base if np.isfinite(auc_both) and np.isfinite(auc_base) else float("nan"),
                "gap_geneformer_minus_scgpt": auc_gf - auc_sc if np.isfinite(auc_sc) and np.isfinite(auc_gf) else float("nan"),
                "both_minus_geneformer": auc_both - auc_gf if np.isfinite(auc_both) and np.isfinite(auc_gf) else float("nan"),
            }
    ...
    detail_df.to_csv(output_dir / "external_lung_seed_ensemble_bundle_detail.csv", index=False)

    seed_only = detail_df[detail_df["mode"] == "single_seed"].copy()
    agg_df = (
        seed_only.groupby("bundle", as_index=False)
        .agg(
    ...
    agg_df.to_csv(output_dir / "external_lung_seed_ensemble_bundle_seed_aggregate.csv", index=False)

    print("[info] detail:")
    print(detail_df.to_string(index=False))
    print("[info] seed aggregate:")
    print(agg_df.to_string(index=False))
    print(f"[done] {output_dir / 'external_lung_seed_ensemble_bundle_detail.csv'}")
    print(f"[done] {output_dir / 'external_lung_seed_ensemble_bundle_seed_aggregate.csv'}")
