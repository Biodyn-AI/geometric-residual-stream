...
results. All new analyses (cycles 43–51) are reproducible from
`implementation/scripts/run_wp*.py` and the shared `revision_lib.py`; a
consolidated summary is in `reports/revision_cycles43-51_new_experiments.md`.
...
New experiment (WP5, cycle47). We added a Methods subsection "Exploratory versus
...
New experiment (WP1, cycle43). We re-evaluated under leave-TF-out, leave-target-
...
edge CV) but **collapses to near-zero under leave-both-out** (+0.001 to +0.003);
...
New experiment (WP2, cycle44). New Methods subsection states the exact protocol
...
We chose to **make the extraction comparable** (option a). New experiment (WP6,
cycle51): we extracted Geneformer per-layer residual-stream activations from real
...
New experiments (WP3, cycle45; WP4, cycle46). Absolute AUROC and AUPRC are now in
...
Methods subsections for every new analysis. The `implementation/README.md` lists
reproduction commands for all new cycles.
...
Done (WP3). Every headline table now reports baseline-only and geometry-augmented
absolute AUROC. Baseline AUROC ranges 0.50–0.65; the geometry-augmented AUROC
(0.58–0.69) is always above both 0.5 and the baseline, so the deltas are genuine
improvements over a meaningful starting point.

**2. Actual AUROC for usability; envision how to sift the signal.**
New experiment (WP10, cycle49). New Results subsection reports top-k retrieval:
...
panel, and precision/recall@k in the new retrieval analysis (WP10). Prevalence is
...
New experiment (WP9, cycle50). New Results subsection and Table "Geometric signal
...
New experiment (WP8, cycle48). The reviewer is correct that cosine/centered
...
- All new code and per-cycle outputs added to the public repository.
