...
\subsection*{Residual-stream geometry carries regulatory signal beyond expression confounds}
...
The null controls were run at layer~5, where the raw-cosine effect is near its peak (layers~4 and~5 are essentially tied).
Label-permutation analysis ($n = 40$) yielded an empirical $p$-value of 0.025 at layer~5, indicating that only 1 of 40 random label assignments produced a $\DAURC$ as large as the observed value.
...
A sensitivity analysis on token budget (512 vs.\ 1,024 genes per cell) revealed that the geometric effect attenuates modestly with larger budgets: raw-cosine $\DAURC$ at L4 decreased from $+0.076$ at the 512-gene budget to $+0.066$ at 1,024 genes, remaining positive with a bootstrap CI above zero.
...
The immune domain showed consistent positive gains under raw cosine (Table~\ref{tab:cross_domain}; $\DAURC = +0.024$ at seed 42, with all three independent seed replicates above zero), but lung and external lung initially appeared near-null under the same cosine similarity protocol that succeeded in kidney.
...
\end{tabular}
\par\smallskip
\footnotesize Edge-level score correlations between the two models: $\rho = 0.476$ (immune), $0.480$ (lung), $0.467$ (external lung). Geneformer null controls: $p < 0.001$ in every domain. All AUROC values are absolute.
\end{table}
...
This produced a dramatic improvement in external lung, raising the scGPT $\DAURC$ from $+0.003$ (single layer L3) to $+0.026$ (L0--11 bundle), reducing the Geneformer--scGPT gap from $+0.025$ to just $+0.001$ (Figure~\ref{fig:cross_model}B; Table~\ref{tab:bundle}).
...
The same representation repair protocol generalized across domains: in immune, bundled seed-ensemble scGPT ($+0.066$) matched or exceeded the embedding-layer Geneformer reference ($+0.054$); in lung, the pattern was similar ($+0.039$ vs.\ $+0.028$).
This particular comparison, however, is itself asymmetric in the opposite direction---it bundles scGPT across layers while still representing Geneformer by a single embedding layer; a fully matched comparison, in which both models receive identical multi-layer extraction, is presented in a later subsection and shows the two models to be close, with Geneformer modestly ahead in most domains.

These findings demonstrate that the initial scGPT disadvantage was not a reflection of inferior model quality but largely of a suboptimal extraction protocol: selecting a single representational layer discards information distributed across the transformer's processing depth.
...
\subsection*{Combined modeling yields robust improvements under nested cross-validation}
...
(Leakage from shared gene identity is a separate concern, addressed by the grouped cross-validation in the following subsection.)
...
\begin{table}[t]
\centering
\caption{\textbf{Compact stacking model under nested outer-split cross-validation.}
...
\caption{\textbf{Compact stacking model performance under nested cross-validation.}
...
All evaluations use nested outer-split cross-validation to prevent information leakage between feature construction and evaluation.
...
\subsection*{Grouped cross-validation delimits the geometric signal}
...
Under the stricter \emph{leave-both-out} split, however, the signal collapsed to near-zero ($\DAURC$ of $0$ to $+0.003$ across domains; Table~\ref{tab:grouped}).
...
Under edge-level CV the compact model improved over the best single branch by $+0.003$ to $+0.014$ AUROC, broadly in line with the gains in Table~\ref{tab:compact}; under leave-both-out CV this advantage shrank to $\pm0.001$, mirroring the behaviour of the underlying geometric signal.
...
Absolute baseline-only and geometry-augmented AUROC, and $\DAURC$, for the centered-cosine multi-layer bundle applied identically to scGPT and to Geneformer residual-stream activations (single-seed extraction; the seed-ensemble scGPT values in Table~\ref{tab:bundle} are correspondingly slightly higher).
...
Despite methodological recovery, effect sizes in lung were consistently smaller than in immune or kidney: under the centered-cosine bundle the lung $\DAURC$ was $+0.028$, against $+0.042$ (immune) and $+0.122$ (kidney).
...
The central finding of this work is that the geometric arrangement of gene vectors in foundation model representation spaces carries incremental information about transcription factor--target regulatory relationships, beyond expression-level confounds.
This signal is reproducible across two independently trained architectures, but it is bounded: as the grouped cross-validation shows, it supports retrospective re-ranking of edges within a known gene universe rather than de novo prediction among unseen genes.
...
Perhaps the most important methodological insight is that regulatory information in transformer residual streams is distributed across layers and varies stochastically across random cell samples.
The marked improvement from single-layer to multi-layer bundled scGPT representations---from near-null to a clear positive signal in the external-lung domain---demonstrates that different layers encode qualitatively different aspects of regulatory geometry.
...
TRRUST edges are directed (TF $\rightarrow$ target) and annotated with a mode of regulation (activation, repression, or unknown); we used the TF$\rightarrow$target direction but not the activation/repression sign.
After mapping to each tissue's gene universe, the positive (and total, positive plus negative) edge counts were: kidney 288 (1{,}152), immune 735 (2{,}940), lung 8{,}195 (32{,}780), and external lung 7{,}504 (30{,}016).

\paragraph{Negative edge construction.}
For each domain, we constructed matched negative edges by sampling random (source~TF, target~gene) pairs from the same source and target gene pools as the positive edges, with a 3:1 negative-to-positive ratio in every domain.
...
Edge-level predictive evaluations used repeated stratified 5-fold cross-validation (3 repeats); grouped evaluations used the leave-TF / leave-target / leave-both-out splits described below.
For each $\DAURC$ estimate, 95\% bootstrap confidence intervals were computed from at least 400 resamples (1,000 for the headline grouped, confirmatory and cross-model analyses).
...
The primary negative set is sampled once by uniform rejection sampling of (source-pool TF, target-pool gene) pairs excluding positives, at a 3:1 negative-to-positive ratio in every domain.
For the robustness analysis we additionally generated negatives under two harder protocols at a balanced 1:1 ratio (chosen so that the harder-negative comparison is not confounded by class ratio): \emph{degree-matched}, in which each negative's TF and target are drawn from the same TRRUST out-degree / in-degree quantile bin (5 bins) as the corresponding positive; and \emph{expression-matched}, matched instead on mean-expression and detection-frequency bins.
...
We used the pre-trained Geneformer model (\texttt{ctheodoris/Geneformer}; 18-layer transformer, hidden dimension 1{,}152) and sampled 192 cells per domain.
...
\textit{Nucleic Acids Res}. 2018;46(D1):D380--D386.
